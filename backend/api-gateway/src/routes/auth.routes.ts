...
const options = {
  ...
  onProxyReq: (proxyReq: any, req: any) => {
    ...
    console.log(`[Auth Proxy] Forwarding ${req.method} request to ${config.authServiceUrl}${req.url}`);
  },
  onProxyRes: (proxyRes: any, req: any) => {
    // Log successful proxy response
    console.log(`[Auth Proxy] Received ${proxyRes.statusCode} response for ${req.method} ${req.url}`);
  },
  onError: (err: Error, req: any, res: any) => {
    console.error(`[Auth Proxy] Error proxying ${req.method} ${req.url}:`, err.message);
